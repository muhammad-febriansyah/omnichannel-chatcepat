...
func (r *Repository) ListRules(ctx context.Context, tenantID string) ([]AutoReplyRule, error) {
	rows, err := r.pool.Query(ctx, `
		...
		WHERE sa.tenant_id=$1 ORDER BY ar.priority DESC, ar.created_at ASC
	`, tenantID)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) loadRuleChildren(ctx context.Context, rule *AutoReplyRule) error {
	keywordRows, err := r.pool.Query(ctx, `SELECT keyword FROM auto_reply_rule_keywords WHERE auto_reply_rule_id=$1 ORDER BY created_at`, rule.ID)
	if err != nil {
		return err
	}
	...
	actionRows, err := r.pool.Query(ctx, `
		...
		FROM auto_reply_rule_actions WHERE auto_reply_rule_id=$1 ORDER BY sort_order, created_at
	`, rule.ID)
	if err != nil {
		return err
	}
	for actionRows.Next() {
		...
		responseRows, err := r.pool.Query(ctx, `SELECT content FROM auto_reply_action_responses WHERE auto_reply_rule_action_id=$1 AND is_active=true ORDER BY created_at`, action.ID)
		if err != nil {
			actionRows.Close()
			return err
		}
...
func (r *Repository) CreateRule(ctx context.Context, tenantID, accountID string, input RuleInput) (AutoReplyRule, error) {
	...
	err = tx.QueryRow(ctx, `
		...
		SELECT id, $3, $4, $5, $6, $7, $8 FROM social_accounts WHERE tenant_id=$1 AND id=$2
...
func (r *Repository) UpdateRule(ctx context.Context, tenantID, ruleID string, input RuleInput) (AutoReplyRule, error) {
	...
	result, err := tx.Exec(ctx, `
		...
		FROM social_accounts sa WHERE ar.social_account_id=sa.id AND sa.tenant_id=$7 AND ar.id=$8
	...
	if _, err := tx.Exec(ctx, `DELETE FROM auto_reply_rule_keywords WHERE auto_reply_rule_id=$1`, ruleID); err != nil {
		return AutoReplyRule{}, err
	}
	if _, err := tx.Exec(ctx, `DELETE FROM auto_reply_rule_actions WHERE auto_reply_rule_id=$1`, ruleID); err != nil {
		return AutoReplyRule{}, err
	}
...
func (r *Repository) DeleteRule(ctx context.Context, tenantID, ruleID string) error {
	result, err := r.pool.Exec(ctx, `
		...
		WHERE ar.social_account_id=sa.id AND sa.tenant_id=$1 AND ar.id=$2
	`, tenantID, ruleID)
	if err != nil {
		return err
	}
...
func (r *Repository) SetRuleActive(ctx context.Context, tenantID, ruleID string, active bool) error {
	result, err := r.pool.Exec(ctx, `
		...
		FROM social_accounts sa WHERE ar.social_account_id=sa.id AND sa.tenant_id=$2 AND ar.id=$3
...
func insertRuleChildren(ctx context.Context, tx pgx.Tx, ruleID string, input RuleInput) error {
	for _, keyword := range input.Keywords {
		if _, err := tx.Exec(ctx, `INSERT INTO auto_reply_rule_keywords (auto_reply_rule_id, keyword) VALUES ($1,$2)`, ruleID, NormalizeContent(keyword)); err != nil {
			return err
		}
	}
	for i, action := range input.Actions {
		...
		var actionID string
		if err := tx.QueryRow(ctx, `INSERT INTO auto_reply_rule_actions (auto_reply_rule_id, action_type, content, sort_order, is_active) VALUES ($1,$2,$3,$4,$5) RETURNING id::text`, ruleID, action.ActionType, action.Content, sortOrder, action.IsActive).Scan(&actionID); err != nil {
			return err
		}
		for _, response := range action.Responses {
			if _, err := tx.Exec(ctx, `INSERT INTO auto_reply_action_responses (auto_reply_rule_action_id, content) VALUES ($1,$2)`, actionID, response); err != nil {
				return err
			}
		}
	}
...
func (r *Repository) GetSettings(ctx context.Context, accountID string) (AutomationSettings, error) {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO social_account_settings (social_account_id) VALUES ($1) ON CONFLICT (social_account_id) DO NOTHING
	`, accountID)
	if err != nil {
...
func (r *Repository) scanSettings(ctx context.Context, accountID string) (AutomationSettings, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		FROM social_account_settings WHERE social_account_id=$1
...
func (r *Repository) UpdateSettings(ctx context.Context, tenantID, accountID string, settings AutomationSettings) (AutomationSettings, error) {
	result, err := r.pool.Exec(ctx, `
		...
		reply_cooldown_seconds, max_consecutive_errors)
		SELECT $2,$3,$4,$5,$6,$7,$8,GREATEST($9,0),GREATEST($10,1)
...
func (r *Repository) ListIgnoreKeywords(ctx context.Context, accountID string) ([]string, error) {
	rows, err := r.pool.Query(ctx, `SELECT keyword FROM social_account_ignore_keywords WHERE social_account_id=$1 ORDER BY created_at`, accountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repository) RescheduleJob(ctx context.Context, jobID string, scheduledAt time.Time) error {
	_, err := r.pool.Exec(ctx, `UPDATE social_jobs SET status='queued', scheduled_at=$1, started_at=NULL, updated_at=now() WHERE id=$2`, scheduledAt, jobID)
	return err
}

func (r *Repository) SetEventStatus(ctx context.Context, eventID, status string, ruleID *string) error {
	_, err := r.pool.Exec(ctx, `UPDATE social_incoming_events SET status=$1, matched_rule_id=$2, updated_at=now() WHERE id=$3`, status, ruleID, eventID)
	return err
}

func (r *Repository) CreateEventAction(ctx context.Context, eventID, actionType string) (string, bool, error) {
	var id string
	err := r.pool.QueryRow(ctx, `INSERT INTO social_event_actions (social_incoming_event_id, action_type) VALUES ($1,$2) ON CONFLICT (social_incoming_event_id, action_type) DO NOTHING RETURNING id::text`, eventID, actionType).Scan(&id)
	if errors.Is(err, pgx.ErrNoRows) {
		return "", false, nil
	}
...
func (r *Repository) GetEventAction(ctx context.Context, eventID, actionType string) (string, string, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		WHERE social_incoming_event_id=$1 AND action_type=$2
...
func (r *Repository) GetAutomationJobForEventAction(ctx context.Context, eventID, actionType string) (Job, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		FROM social_jobs
		WHERE source_event_id=$1 AND action=$2
...
func (r *Repository) SetEventActionStatus(ctx context.Context, actionID, status string, errMessage *string) error {
	_, err := r.pool.Exec(ctx, `UPDATE social_event_actions SET status=$1, executed_at=CASE WHEN $1 IN ('completed','failed') THEN now() ELSE executed_at END, error_message=$2, updated_at=now() WHERE id=$3`, status, errMessage, actionID)
	return err
}
...
func (r *Repository) SetEventActionStatusByEvent(ctx context.Context, eventID, actionType, status string, errMessage *string) error {
	_, err := r.pool.Exec(ctx, `UPDATE social_event_actions SET status=$1, executed_at=CASE WHEN $1 IN ('completed','failed') THEN now() ELSE executed_at END, error_message=$2, updated_at=now() WHERE social_incoming_event_id=$3 AND action_type=$4`, status, errMessage, eventID, actionType)
	return err
}
