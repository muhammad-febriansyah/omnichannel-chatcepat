...
func (p *Provider) Platform() string         { return social.PlatformFacebook }
...
func (p *Provider) CheckSession(ctx context.Context, _ social.Account) error {
	...
	info, _ := page.Info()
	if info != nil && (strings.Contains(strings.ToLower(info.URL), "/login") || strings.Contains(strings.ToLower(info.URL), "/checkpoint")) {
		return social.ErrLoginRequired
	}
	if body, err := page.Element("body"); err == nil {
		text, _ := body.Text()
		lower := strings.ToLower(text)
		if strings.Contains(lower, "log in") || strings.Contains(lower, "masuk") {
			return social.ErrLoginRequired
		}
	}
...
func (p *Provider) ReplyComment(ctx context.Context, _ social.Account, event social.IncomingSocialEvent, content string) error {
...
func (p *Provider) SendPrivateReply(ctx context.Context, account social.Account, event social.IncomingSocialEvent, content string) error {
	if event.SourceType != social.EventSourceComment || event.AuthorExternalID == accountExternalID(account) || event.TargetURL == nil || strings.TrimSpace(*event.TargetURL) == "" {
...
func pageURLForNode(node *rod.Element, fallback string) string {
	...
	if strings.Contains(fallback, "/posts/") || strings.Contains(fallback, "/permalink/") || strings.Contains(fallback, "/photos/") {
		return fallback
	}
	return ""
}

func submitReply(page *rod.Page, inputSelectors, submitSelectors, replySelectors []string, content string) error {
	if replySelectors != nil {
		if button, err := findFirst(page, replySelectors); err == nil {
			if err := button.Click(proto.InputMouseButtonLeft, 1); err != nil {
